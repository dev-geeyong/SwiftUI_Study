...
struct ContentView: View {
   ...
   @State var cardFaceUps: [Bool]
   @State var themeColor = Color.orange  // 테마 색상 추가
   
   init() {
       _cardFaceUps = State(initialValue: Array(repeating: false, count: (theme1 + theme1).count))
       _emojis = State(initialValue: (theme1 + theme1).shuffled())  // 초기 카드 셔플
   }
