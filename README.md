...
# SwiftUI 기반의 iOS 프로그래밍 - Widget 파트

### [CH 56.0] 용어정리

- **엔트리**: 위젯의 데이터 모델, 특정 시점에 표시할 데이터

---

### [CH 56.1-3] 위젯 구성 요소

위젯은 다음 세 개 항목으로 이루어져 있음:

- **위젯 종류**: 위젯 식별
- **위젯 구성**: 프로토콜 준수하는 선언
  - 표시할 정보, 디스플레이 이름과 설명, 위젯 지원 크기
  - 정적 구성과 인텐트 구성 지원
    - **인텐트 구성**: 사용자가 위젯을 구성할 수 있도록 함  
      예: 사용자가 위젯 내에서 표시할 헤드라인 직접 선택
    - **정적 구성**: 위젯에 사용자가 구성할 수 있는 속성이 없음
- **엔트리 뷰**: 위젯의 레이아웃
...
- 위젯의 레이아웃, 크기에 따라 다른 레이아웃 가능
- 단순한 SwiftUI 뷰
- 위젯은 일반적으로 non-interactive 이기에 버튼, 슬라이더, 토글 등이 없음
...
- 위젯 데이터를 리로드 해줄 수 있음
- "시점"과 해당 시점에 보여줄 "데이터"가 있으면 됨

---

### [CH 56.7-8] 프로바이더 및 리로드 정책

- 타임라인 프로바이더 프로토콜을 준수하려면 다음 두 개 메서드를 구현해야 함:
  - **Snapshot**: 미리보기 느낌
  - **timeline**: 리로드 로직
    - `Timeline` 메서드 내에서 세 가지 리로드 로직을 사용할 수 있음:
      1. **atEnd**
         - 마지막 엔트리 이후 자동으로 데이터 로드 (주기적으로 업데이트가 필요한 위젯)
         - 예: 1시간마다 새로운 데이터를 표시하는 위젯이라면, 엔트리를 1시간 단위로 생성하고 마지막 엔트리가 끝날 때 데이터를 다시 가져오게 설정
      2. **after**
         - 지정한 시간(Date) 이후에 데이터를 다시 로드. 사용자가 원하는 정확한 시점을 지정할 수 있음
         - 특정 시간 이후에만 데이터를 새로고침해야 하는 경우  
           예: 매일 특정 시간(아침 9시)에 데이터를 업데이트해야 하는 위젯
      3. **never**
         - 자동으로 새 데이터를 로드하지 않음. 앱이 강제로 업데이트를 트리거하지 않는 한, 위젯에 새 데이터를 가져오지 않음
         - 정적 콘텐츠를 표시하거나 업데이트가 필요 없는 경우

---

### [CH 56.9] 관련성

- iOS의 스마트 스택, 위젯 갤러리에서 사용: 어떤 위젯이 중요해서 위에 띄워줘야 하는지 OS가 판단하는 데 활용하는 지표
  - **score (Float)**
    - 중요도를 나타내는 점수로, 0.0에서 1.0 사이의 값을 가짐
    - 값이 높을수록 중요도가 높아지고, 시스템이 해당 엔트리를 더 우선적으로 노출하려고 함
    - 예: 날씨 경고 위젯에서 중요한 경고가 있다면 높은 점수를 부여
  - **duration (TimeInterval)**
    - 해당 엔트리가 얼마나 오랫동안 유효한지를 나타냄
    - 예: 이벤트 시작 전 1시간 동안 엔트리를 중요하게 노출하려면 duration을 3600(초 단위)으로 설정
- 반드시 구현해야 하는 건 아님, 스택에서 위로 올리는 걸 판단할 때 OS를 도와줄 뿐

---

### [CH 56.10] 타임라인 리로드 강제 실행

- 앱 내에서 `WidgetCenter` 인스턴스의 `reloadTimeLines`를 실행해서 강제 리로드 할 수 있음

---

### [CH 56.11] 위젯 크기

- 위젯을 소형, 중형, 대형 크기로 표시할 수 있음
...
- 플레이스홀더를 제공해야 함
- "가려진" 상태를 `redacted`로 표현할 수 있음 (skeleton view 같은 것)
